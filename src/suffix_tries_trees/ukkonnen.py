...
class Node:
    ...
    def get_edge_with_prefix(self, prefix: str, tree: "Tree") -> "Reference | str | None":
    ...
    def get_label(self, tree: "Tree") -> str:
        if self.parent is None:
            ...
            edges = [tree.resolve(edge) for edge in self.parent.children.keys() if self.parent.children[edge] == self]
...
class Tree:
    ...
    def test_and_split(self, node: Node, reference: Reference, x: str) -> Tuple[bool, Node]:
        self.log()
    ...
    def update(self, s: Node, reference: Reference, i: int) -> Tuple[Node, int]:
        ...
        if old_r is not self.root:
            self.log(f"Setting suffix link of {old_r.to_string(self)} to {r.to_string(self)}")
            old_r.suffix_link = r
