...
def test_shift_table_relations():
    ...
    for word in words:
        assert __compute_shift_table_z__(word) == __compute_shift_table_z_mathematical__(word)
        assert all(with_z <= without_z for with_z, without_z in zip(__compute_shift_table_z__(word), __compute_shift_table__(word)))
